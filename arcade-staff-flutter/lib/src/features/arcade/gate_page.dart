...
class _ArcadeGatePageState extends ConsumerState<ArcadeGatePage> {
  ...
  Future<void> _approve() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gate check-in approved')));
      await _lookup();
    } catch (e) {
  ...
  Future<void> _reject() async {
    ...
    try {
      ...
      await HapticFeedback.mediumImpact();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Entry rejected')));
      await _lookup();
    } catch (e) {
      await HapticFeedback.vibrate();
      if (!mounted) return;
  ...
  Future<bool> _confirm({required String title, required String message, String confirmText = 'Confirm'}) async {
  ...
  Widget build(BuildContext context) {
    final item = _item;
    return Scaffold(
      appBar: AppBar(title: const Text('Arcade Gate')),
      body: Stack(
        children: [
          AppBackdrop(
            child: ListView(
              ...
              children: [
          const SectionHeader(title: 'Gate Check-In', subtitle: 'Temporary event module'),
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Wallet code'),
          ),
          ...
          if (item != null) ...[
            ...
            SurfaceCard(
              child: Column(
                  ...
                  children: [
                    Text(item['name']?.toString() ?? '-', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 18)),
                    const SizedBox(height: 8),
